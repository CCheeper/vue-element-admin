<template>
  <div class="app-container">
    ...
    >
      ...
      <el-table-column label="标题" min-width="110px" align="center">
        <template slot-scope="scope">
          <span>{{ scope.row.title }}</span>
        </template>
      </el-table-column>

      <el-table-column label="信息" min-width="160px" align="center">
        <template slot-scope="scope">
...
export default {
  ...
  methods: {
    ...
    handleFilter() {
      ...
      var title;
      this.listQuery.title = this.listQuery.username;
      this.getList(title);
    },
    ...
    handleCreate() {
      ...
      var this_ = this;
        axios
        .get("/workdynamic/getthings")
        .then(function(response) {
          this_.temp.id = response.data.id;
          this_.temp.lasttime = response.data.date;
          this_.temp.edtiorid = Cookies.get("username");
        })
        .catch(function(error) {
          console.log(error);
        } );
    ...
    updateData() {
      this.$refs["dataForm"].validate(valid => {
        if (valid) {
          ...
          updateWorkdynamic(tempData).then(() => {
